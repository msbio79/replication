...
function getReplicationState(time) {
    ...
    // Stage 3: Synthesis 1 (Leading strand continuous synthesis & Lagging strand Fragment 1)
        ...
        // Fill top leading strand up to and including polymerase position
        ...
        const dna1Limit = Math.floor(lerp(10, -1, t));
        for (let i = 10; i >= dna1Limit; i--) {
            if (i >= 0) state.bottomReplication[i] = 'DNA';
        }
    ...
    // Stage 4: Synthesis 2 & 3 (Leading strand continues & Lagging strand Fragment 2 & 3)
        ...
        if (time < 570) {
            ...
            const dna2Limit = Math.floor(state.polyBottom.x / BASE_SPACING);
            ...
            const dna3Limit = Math.floor(state.polyBottom.x / BASE_SPACING);
    ...
    for (let i = 0; i < NUM_BASES; i++) {
        ...
        if (type) {
            ...
            if (!drawingNew || color !== lastColor) {
                if (drawingNew) {
                    // Extend previous segment to this point to close the gap, then stroke
...
function setupInteractions() {
    ...
    canvas.addEventListener('touchstart', (e) => {
        if (e.touches.length === 1) {
            ...
            // Record the midpoint in screen coordinates at pinch start
            touchStartMid.x = (e.touches[0].clientX + e.touches[1].clientX) / 2;
            touchStartMid.y = (e.touches[0].clientY + e.touches[1].clientY) / 2;
            // Also store the world coordinate of the pinch midpoint (pivot)
            const rect0 = canvas.getBoundingClientRect();
            const pivotLocalX = touchStartMid.x - rect0.left;
            const pivotLocalY = touchStartMid.y - rect0.top;
            touchStartMid.worldX = (pivotLocalX - transform.x) / transform.scale;
            touchStartMid.worldY = (pivotLocalY - transform.y) / transform.scale;
        }
    });
    ...
    canvas.addEventListener('touchmove', (e) => {
        if (isDragging && e.touches.length === 1) {
            ...
            const newScale = Math.min(Math.max(touchStartScale * zoomFactor, 0.25), 3.0);

            const rect = canvas.getBoundingClientRect();
            // Current local (canvas-relative) midpoint
            const currentLocalMidX = currentMidX - rect.left;
            const currentLocalMidY = currentMidY - rect.top;

            // Keep the world-space pivot point fixed under the current finger midpoint
            transform.scale = newScale;
            transform.x = currentLocalMidX - touchStartMid.worldX * newScale;
            transform.y = currentLocalMidY - touchStartMid.worldY * newScale;
